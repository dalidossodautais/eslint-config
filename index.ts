...
import noEmptyLinesInObjectsRule from './rules/no-empty-lines-in-objects.js';
import noEmptySxRule from './rules/no-empty-sx.js';
import noNonJSDocCommentsRule from './rules/no-non-jsdoc-comments.js';
...
const getReactRules = (): Record<string, Linter.RuleEntry> => ({
...
const getJestRules = (): Record<string, Linter.RuleEntry> => ({
...
const createConfig = (options: ConfigOptions = { jest: false, react: false }): Linter.Config[] => {
  const { jest: enableJest = false, react = false } = options;
  ...
  const baseRules: Record<string, Linter.RuleEntry> = {
    ...
    'custom-rules/no-empty-lines-in-objects': 'error',
    'custom-rules/no-non-jsdoc-comments': 'error',
  ...
  const plugins: Record<string, unknown> = {
    ...
    'custom-rules': {
      rules: {
        'no-empty-lines-in-objects': noEmptyLinesInObjectsRule,
        'no-non-jsdoc-comments': noNonJSDocCommentsRule,
  ...
  const config: Linter.Config[] = [
    ...
    },
    {
      ...
      plugins: plugins as unknown as Linter.Config['plugins'],
      rules: baseRules,
      ...(react
        ? {
            settings: {
      ...
      ? [
          {
            ...
            languageOptions: {
              globals: {
                ...(jest.environments?.globals?.globals as Record<string, unknown>),
                ...globals.node
              },
              parser: typescriptParser,
              parserOptions: {
...
const config = createConfig({ jest: false, react: false });
